...
def direct_image_router(user_text: str) -> Optional[List[str]]:
    t = normalize_text(user_text)

    # 1. Зогсоолын зургууд
    parking_kws = ["зогсоол", "дулаан зогсоол", "машины зогсоол", "б1 зогсоол", "нэгдсэн зогсоол"]
    if match_any(parking_kws, t):
        if match_any(["план", "төлөвлөлт"], t):
            return ["MULT_PARKING_SPACE"]
        if match_any(["харагдах", "гадаад", "үзэмж"], t):
            return ["MULT_PARKING_SPACE_1"]
        if match_any(["зураг", "үзье", "үзмээр", "харья", "авья"], t):
            return ["MULT_PARKING_SPACE", "MULT_PARKING_SPACE_1"]

    # 2. Таун хаус тусгай хэмжээний зургууд
    if match_any(["267", "266"], t) and match_any(["зураг", "план", "төлөвлөлт", "үзье", "харья", "авья"], t):
    ...
    if match_any(["мульт", "мульт хаус", "мультхаус"], t):
    ...
    # 6. Ногоон байгууламж / Орчин
    if match_any(["ногоон байгууламж", "ногоон цэцэрлэг", "ногоон орчин"], t) and match_any(["зураг", "үзье", "харья"], t):
        return ["GREEN_GARDEN"]

    if match_any(["тохижилт"], t) and match_any(["зураг", "үзье", "харья"], t):
        return ["LANDSCAPING"]

    if match_any(["амрах талбай", "амралтын талбай"], t) and match_any(["зураг", "үзье", "харья"], t):
        return ["RELAXATION_AREA"]

    # 7. Хүүхдийн / Спортын талбай
    if match_any(["0-5", "0 5", "0-5 нас"], t) and match_any(["зураг", "талбай", "үзье", "харья"], t):
        return ["SPORTS_AREA_0_5"]

    if match_any(["9-13", "9 13", "9-13 нас"], t) and match_any(["зураг", "талбай", "үзье", "харья"], t):
        return ["SPORTS_AREA_9_13"]

    if match_any(["13-16", "13 16", "13-16 нас"], t) and match_any(["зураг", "талбай", "үзье", "харья"], t):
        return ["SPORTS_AREA_13_16"]

    if match_any(["спортын талбай"], t) and match_any(["план", "төлөвлөлт", "зураг", "үзье", "харья"], t):
        return ["SPORTS_AREA_PLAN"]

    # 8. Ерөнхий төлөвлөгөө / Хотхоны талбай
    general_keywords = [
        "талбайн зураг", "талбай зураг", "ерөнхий төлөвлөгөө", "ерөнхий план",
        "план зураг", "план", "төлөвлөлтийн зураг", "төлөвлөлт зураг",
        "хотхоны зураг", "хотхон зураг", "нийт зураг"
    ]
    if match_any(general_keywords, t):
        if not match_any(["таун", "мульт", "зогсоол", "спорт", "тоглоом", "ногоон", "тохижилт", "амрах"], t):
            return ["GENERAL_PLAN"]
...
def direct_faq_router(user_text: str) -> Optional[str]:
    t = normalize_text(user_text)
    ...
    # Сингл / Твин хаус (Борлуулалт дууссан)
    if match_any(["сингл", "сингл хаус", "твин", "твин хаус", "ганц айлын", "хоёр айлын"], t):
        return (
    ...
    # Нийт мкв сонголтууд
    if match_any(["мкв сонголт", "м2 сонголт", "мкв", "мкв-ын сонголт", "талбайн сонголт", "хэмжээний сонголт", "ямар сонголт", "ямар мкв", "ямар м2"], t):
        return (
    ...
    # Мульт хаус ерөнхий текст
    if match_any(["мульт хаус", "мультхаус", "мульт"], t):
        mult_sizes = ["100", "116", "120", "125", "126", "136", "178", "189", "192", "198"]
        if not match_any(mult_sizes, t):
            return (
    ...
    # Таун хаус ерөнхий текст
    if match_any(["таун хаус", "таунхаус", "таун"], t):
        if not match_any(["212", "213", "266", "267"], t):
            return (
    ...
    # Үнэ
    if match_any(["м2 хэд", "м2 хэд вэ", "м2 үнэ", "м2 үнэ хэд", "1м2", "1 м2", "квадратын үнэ", "квадрат үнэ", "үнэ хэд", "үнэ хэд вэ", "хэдэн төгрөг", "м2 нь"], t):
        return (
    ...
    # Борлуулалтын утас
    if match_any(["утас", "холбогдох", "холбоо барих", "дугаар", "утасны дугаар"], t):
        return "Манай борлуулалтын утас: 9430-7017 😊"

    # Борлуулалтын оффис
    if match_any(["оффис хаана", "оффисын хаяг", "борлуулалтын оффис", "оффис"], t):
        return f"Борлуулалтын оффис: {SALES_OFFICE}. Утас: {SALES_PHONE} 😊"

    # Байршил
    if match_any(["хаана байрладаг", "хаана байрлах", "байршил", "байрлал", "хаана байдаг", "хаана вэ", "хотын хаана"], t):
        return f"Miners Villa нь {LOCATION_TEXT}. Дэлгэрэнгүй мэдээллийг {SALES_PHONE} дугаараас лавлаарай 😊"

    # Төлбөрийн нөхцөл
    if match_any(["төлбөрийн нөхцөл", "төлөлтийн нөхцөл", "хэрхэн төлөх", "яаж төлөх", "урьдчилгаа", "төлбөр хэдэн хувь", "хэдэн хувь төлөх"], t):
        return f"Төлбөрийн нөхцөл: {PAYMENT_TEXT} Явцын төлбөрт зөвхөн байрны бартер сонсоно."

    # Бартер
    if match_any(["бартер", "байраар төлөх", "машинаар төлөх", "газраар төлөх"], t):
        if match_any(["машин", "машинаар"], t):
            return "Явцын төлбөрт зөвхөн байрны бартер сонсоно. Машины бартер зөвшөөрөхгүй."
        if match_any(["газар", "газраар"], t):
    ...
    # Таун хаусын талбай
    if match_any(["таун хаус хэдэн м2", "таун хаусын талбай", "таун хаусын хэмжээ", "таунхаус хэдэн м2", "таунхаусын талбай"], t):
        return "Таун хаусын сонголтууд 213.33 м² болон 267.48 м² талбайтай."

    # Мульт хаусын талбай
    if match_any(["мульт хаус хэдэн м2", "мульт хаусын талбай", "мульт хаусын хэмжээ", "мультхаусын талбай"], t):
        return "Мульт хаусын талбайнууд: 126, 125.21, 192.25, 189.64, 136.42, 178.39, 198.52 болон 189.52 м²."

    # Мульт хаусын тусгай хэмжээнүүд
    mult_size_answers = {
        "126.32": "126.32 м²", "126,32": "126.32 м²", "126": "126 м²",
        "125.21": "125.21 м²", "125,21": "125.21 м²", "120.85": "120.85 м²",
        "120,85": "120.85 м²", "116": "116 м²", "100.77": "100.77 м²",
        "100,77": "100.77 м²", "136.42": "136.42 м²", "136,42": "136.42 м²",
        "178.39": "178.39 м²", "178,39": "178.39 м²", "189.64": "189.64 м²",
        "189,64": "189.64 м²", "189.52": "189.52 м²", "189,52": "189.52 м²",
        "192.25": "192.25 м²", "192,25": "192.25 м²", "198.52": "198.52 м²",
        "198,52": "198.52 м²",
    }
    if match_any(["мульт"], t):
        for size, display_size in mult_size_answers.items():
            if size in t:
                return f"Мульт хаусын {display_size} сонголт байна. Зураг үзэх бол \"зураг үзье\" гэж бичээрэй 😊"

    # Дулаан зогсоол
    if match_any(["зогсоолын үнэ", "зогсоол хэд", "зогсоол хэд вэ", "дулаан зогсоолын үнэ", "машины зогсоолын үнэ"], t):
        return PARKING_TEXT
    if match_any(["дулаан зогсоол", "нэгдсэн зогсоол", "б1 зогсоол", "машины зогсоол"], t):
        return PARKING_TEXT

    # Барилгын явц
    if match_any(["барилгын явц", "явц ямар", "барилга хэр явж", "барилга явж байна", "шинэ мэдээ"], t):
    ...
    if match_any(["хэзээ ашиглалтад", "ашиглалтад орох", "хэзээ дуусах", "хэзээ баригдаж дуусах"], t):
        return "2026 оны өвөл гэхэд дотоод заслын ажлыг эхлүүлэхээр ажиллаж байна. Ашиглалтад орох баталгаатай огноо одоогоор өгөөгүй байна."

    # Ажилтантай холбогдох
    if match_any(["хүнтэй ярья", "хүнтэй ярих", "борлуулалтын ажилтан", "борлуулалттай ярья", "менежертэй ярья", "менежер"], t):
        return "😊 Манай борлуулалтын албатай 9430-7017 дугаараар холбогдоорой."
...
def process_ai_response(sender_id: str, user_text: str):
    try:
        ...
        direct_reply = direct_faq_router(user_text)
        image_result = direct_image_router(user_text)
        ...
        # 2. GEMINI AI (Сүлжээний чиглүүлэгт таараагүй бусад асуултад)
